...
def get_dataset(batch_size=200):
 ...
 # features = np.load("X_train_1.npy")
  ...
  features = np.load("/home/rydagostino/data/train/X_train_1.npy")
  filenames = []
  for i in range(2,7,1):
    tmp = np.load("/home/rydagostino/data/train/"+features_name+str(i)+".npy")
    features = np.concatenate((features,tmp))  	
       #filenames.append(os.getcwd()+"/"+features_name+str(i)+".npy")
  
  labels_name = "y_train_"
  labels = np.load("/home/rydagostino/data/train/y_train_1.npy")
  filenames = []
  for i in range(2,7,1):
    tmp = np.load("/home/rydagostino/data/train/"+labels_name+str(i)+".npy")
 ...
 # labels = np.zeros(shape=(7000,1))
  ...
  features = np.load("/home/rydagostino/data/test/X_test_1.npy")
  filenames = []
  for i in range(2,7,1):
    tmp = np.load("/home/rydagostino/data/test/"+features_name+str(i)+".npy")
    features = np.concatenate((features,tmp))       
       #filenames.append(os.getcwd()+"/"+features_name+str(i)+".npy")
  
  labels_name = "y_test_"
  labels = np.load("/home/rydagostino/data/test/y_test_1.npy")
  filenames = []
  for i in range(2,7,1):
    tmp = np.load("/home/rydagostino/data/test/"+labels_name+str(i)+".npy")
    labels = np.concatenate((labels,tmp))
  #test_dataset = tf
...
checkpoint_path = "gs://tpu_models_1/model_1/"
